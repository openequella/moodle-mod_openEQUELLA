...
foreach($DB->get_records('equella') as $resource) {
    ...
    if (curl_exec($ch)) {
        ...
        if ($recipients) {
            ...
            $message = get_string('checker.message', 'equella', array('name' => $resource->name,'url' => $resource->url,'coursename' => $course->shortname,'courseurl' => $CFG->wwwroot . '/course/view.php?id=' . $course->id));
        ...
        if ($disable_links) {
            $cms = $DB->get_records_sql("SELECT cm.* FROM {course_modules} cm
                INNER JOIN {modules} m ON cm.module = m.id
                WHERE m.name = :modname
                AND cm.course = :course
                AND cm.instance = :resource", array('modname' => 'equella','course' => $resource->course,'resource' => $resource->id));
...
function get_course_editors($courseid) {
    ...
    $course_editors = $DB->get_records_sql("SELECT u.* FROM {user} u
        ...
        AND c.id = :courseid
        AND u.deleted = 0
        AND u.emailstop = 0", array('contextlevel' => CONTEXT_COURSE,'capability' => $capability,'courseid' => $courseid));
    if ($course_editors) {
    ...
    $course_editors = $DB->get_records_sql("SELECT u.* FROM {user} u
        ...
        AND c.id = :courseid
        AND u.deleted = 0
        AND u.emailstop = 0", array('contextlevel' => CONTEXT_COURSECAT,'capability' => $capability,'courseid' => $courseid));
    if ($course_editors) {
        return $course_editors;
    }
    ...
    $course_editors = $DB->get_records_sql("SELECT u.* FROM {user} u
        ...
        AND u.deleted = 0
        AND u.emailstop = 0", array('contextlevel' => CONTEXT_SYSTEM,'capability' => $capability));
    if ($course_editors) {
        return $course_editors;
    }
